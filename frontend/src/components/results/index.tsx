    ...
    <div className="fade-in">
      <Section title="◉ Host Info">
        ...
        {d.ssl && (
          <>
            <KVRow
              label="SSL Valid"
              value={d.ssl.valid ? "Yes" : "No"}
              tone={d.ssl.valid ? "good" : "bad"}
            />
            <KVRow label="Expires" value={d.ssl.expires} />
            <KVRow
              label="Days Left"
              value={d.ssl.days_remaining}
              tone={
                d.ssl.days_remaining && d.ssl.days_remaining > 30
                  ? "good"
                  : "bad"
              }
            />
      ...
      <Section title="◉ Security Headers">
        {d.missing_security_headers?.length ? (
          d.missing_security_headers.map((h) => (
            <HintItem key={h} text={`Missing: ${h}`} tone="missing" />
          ))
        ) : (
      ...
      {d.smart_summary?.length ? (
        <Section title="◉ Smart Summary">
          {d.smart_summary.map((s, i) => (
            <SummaryItem key={i} text={s} />
          ))}
        </Section>
      ) : null}
      ...
      {d.next_steps?.length ? (
        <Section title="◉ Next Steps">
          {d.next_steps.map((s, i) => (
            <NextStep key={i} index={i + 1} text={s} />
          ))}
        </Section>
      ) : null}
    </div>
    ...
    <div className="fade-in">
      <Section title="◉ Redirect Chain">
        {d.redirect_chain?.length ? (
          d.redirect_chain.map((hop, i) => (
            <PathRow key={i} path={hop.url} status={hop.status} />
          ))
        ) : (
          <span style={{ color: "var(--text-muted)", fontSize: "0.75rem" }}>
            No redirects.
...
export function BBScanSection({ d }: { d: BBScanResult }) {
  return (
    <div className="fade-in">
      <Section
        title={`◉ Interesting Paths (${d.interesting_paths?.length ?? 0})`}
      >
        {d.interesting_paths?.length ? (
          d.interesting_paths.map((p, i) => (
            <PathRow key={i} path={p.path} status={p.status} />
          ))
        ) : (
          <span style={{ color: "var(--text-muted)", fontSize: "0.75rem" }}>
      ...
      <Section title="◉ All Results" defaultOpen={false}>
        {d.all_results?.map((p, i) => (
          <PathRow key={i} path={p.path} status={p.status} />
        ))}
      </Section>

      <Section title="◉ Bug Bounty Hints">
        {d.bug_bounty_hints?.map((h, i) => (
          <SummaryItem key={i} text={h} />
        ))}
      </Section>
  ...
  return (
    <div className="fade-in">
      <Section title={`◉ ${d.type.toUpperCase()} Payloads (${d.count})`}>
        ...
        {d.payloads.map((p, i) => (
          <PayloadItem key={i} payload={p.payload} label={p.label} />
        ))}
      </Section>

      <Section title="◉ Usage Tips">
        {d.usage_tips.map((t, i) => (
          <NextStep key={i} index={i + 1} text={t} />
        ))}
      </Section>
    </div>
  );
  ...
  return (
    <div className="fade-in">
      <Section title={`◉ Full Workflow — ${d.target}`}>
        ...
        {d.recon?.status_code && (
          <KVRow label="Status" value={d.recon.status_code} />
        )}
      </Section>
      ...
      <Section
        title={`◉ Interesting Paths (${d.bb_scan?.interesting_paths?.length ?? 0})`}
      >
      ...
      <Section title="◉ Misconfigurations">
        {d.analysis?.misconfig_hints?.length ? (
          d.analysis.misconfig_hints.map((h, i) => (
            <HintItem key={i} text={h} />
          ))
        ) : (
  ...
  return (
    <div className="fade-in">
      <Section title={`◉ ${d.question}`}>
        {d.response.map((line, i) => (
          <SummaryItem key={i} text={line} />
        ))}
        {d.tip && (
          <div
            style={{
          ...
          >
            <span style={{ color: "var(--lime)" }}>tip: </span>
            {d.tip}
          </div>
        )}
      </Section>
    ...
    <div className="fade-in">
      <Section title="◉ Overview">
        ...
        <KVRow
          ...
          value={d.endpoints_found}
          tone={d.endpoints_found > 0 ? "good" : undefined}
        />
      </Section>
  ...
  return (
    <div className="fade-in">
      <Section title="◉ Overview">
        ...
        <KVRow
          label="Interesting"
          value={d.interesting.length}
          tone={d.interesting.length > 0 ? "warn" : undefined}
        />
        <KVRow
          label="High risk"
          value={d.high_risk.length}
          tone={d.high_risk.length > 0 ? "bad" : undefined}
        />
      </Section>
              ...
              >
                <span
                  style={{
                    ...
                    letterSpacing: 1,
                    textTransform: "uppercase",
                    border: `1px solid ${riskColor(p.risk)}`,
                    padding: "1px 6px",
                    borderRadius: 3,
                  }}
                >
      ...
      <Section
        title={`◉ All High-Risk Parameters (${d.high_risk.length})`}
        defaultOpen={d.interesting.length === 0}
      >
        {d.high_risk.length ? (
          d.high_risk.map((p, i) => (
            <div
  ...
  return (
    <div className="fade-in">
      <Section title={`◉ Express Workflow — ${d.target}`}>
        ...
        {d.recon?.protocol && (
          <KVRow label="Protocol" value={d.recon.protocol.toUpperCase()} />
        )}
      </Section>
      {d.recon?.ssl && (
        <Section title="◉ SSL">
          <KVRow
            label="Valid"
            value={d.recon.ssl.valid ? "Yes" : "No"}
            tone={d.recon.ssl.valid ? "good" : "bad"}
          />
          <KVRow label="Expires" value={d.recon.ssl.expires} />
          <KVRow
            label="Days"
            value={d.recon.ssl.days_remaining}
            tone={
              d.recon.ssl.days_remaining && d.recon.ssl.days_remaining > 30
                ? "good"
                : "bad"
            }
          />
...
export function BugBountyWorkflowSection({
  d,
}: {
  d: BugBountyWorkflowResult;
}) {
  return (
    <div className="fade-in">
      ...
      {d.bb_scan?.interesting_paths?.length ? (
        <Section
          title={`◉ Interesting Paths (${d.bb_scan.interesting_paths.length})`}
        >
          {d.bb_scan.interesting_paths.map((p, i) => (
...
export function SubdomainsWorkflowSection({
  d,
}: {
  d: SubdomainsWorkflowResult;
}) {
  ...
  return (
    <div className="fade-in">
      <Section title={`◉ Subdomains Workflow — ${d.domain}`}>
        ...
        <KVRow
          label="Live found"
          value={d.expansion?.live_count ?? 0}
          tone={d.expansion?.live_count ? "good" : "warn"}
        />
      ...
      {reconEntries.length > 0 && (
        <Section
          title={`◉ Subdomain Recon Results (${reconEntries.length})`}
          defaultOpen={false}
        >
          {reconEntries.map(([sub, r], i) => (
            ...
            >
              ...
              {r.missing_security_headers?.length ? (
                <HintItem
                  text={`${r.missing_security_headers.length} header(s) missing`}
                />
              ) : null}
            </div>
          ))}
...
export function ApiWorkflowSection({ d }: { d: ApiWorkflowResult }) {
  const apiEps =
    d.endpoints?.endpoints?.filter((ep) => ep.type === "api") ?? [];
  const adminEps =
    d.endpoints?.endpoints?.filter((ep) => ep.type === "admin") ?? [];
  const sensitiveEps =
    d.endpoints?.endpoints?.filter((ep) => ep.type === "sensitive") ?? [];
  ...
  return (
    <div className="fade-in">
      <Section title={`◉ API Scan Workflow — ${d.target}`}>
        ...
        <KVRow
          label="API endpoints"
          value={d.api_endpoints_found}
          tone={d.api_endpoints_found > 0 ? "good" : undefined}
        />
        <KVRow
          label="Total endpoints"
          value={d.endpoints?.endpoints_found ?? 0}
        />
        <KVRow
          label="Injectable params"
          value={d.params?.interesting?.length ?? 0}
          tone={d.params?.interesting?.length ? "warn" : undefined}
        />
      </Section>
      {apiEps.length > 0 && (
        <Section title={`◉ API Endpoints (${apiEps.length})`}>
          {apiEps.map((ep, i) => (
            <PathRow key={i} path={ep.path} status={ep.status} />
          ))}
        </Section>
      )}
      {sensitiveEps.length > 0 && (
        <Section title={`◉ Sensitive Paths (${sensitiveEps.length})`}>
          {sensitiveEps.map((ep, i) => (
            <HintItem key={i} text={`${ep.path} → ${ep.status}`} tone="bad" />
          ))}
        </Section>
      )}
      {adminEps.length > 0 && (
        <Section title={`◉ Admin Panels (${adminEps.length})`}>
          {adminEps.map((ep, i) => (
            <HintItem key={i} text={`${ep.path} → ${ep.status}`} tone="warn" />
          ))}
        </Section>
      )}
      {d.params?.interesting?.length ? (
        <Section
          title={`◉ Injectable Parameters (${d.params.interesting.length})`}
        >
          {d.params.interesting.map((p, i) => (
            ...
            >
              ...
              >
                <span
                  style={{
                    color: p.risk === "high" ? "var(--red)" : "var(--yellow)",
                    fontSize: "0.65rem",
                    textTransform: "uppercase",
                    letterSpacing: 1,
                  }}
                ...
                <code style={{ color: "var(--lime)", fontSize: "0.76rem" }}>
                  ?{p.name}=FUZZ
                </code>
              </div>
              <div
