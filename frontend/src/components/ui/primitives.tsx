...
// Key-value row
export function KVRow({ label, value, tone }: { label: string; value?: string | number | null; tone?: 'good' | 'bad' | 'warn' }) {
  const color = tone === 'good' ? 'var(--lime)' : tone === 'bad' ? 'var(--red)' : tone === 'warn' ? 'var(--yellow)' : 'var(--text)'
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '5px 0', borderBottom: '1px solid var(--border)' }}>
      ...
      <span style={{ color, fontSize: '0.78rem', fontWeight: 500, maxWidth: '65%', textAlign: 'right' as const, wordBreak: 'break-all' as const }}>{value ?? '—'}</span>
    </div>
  )
}

// Hint / warning item
export function HintItem({ text, tone = 'bad' }: { text: string; tone?: 'bad' | 'warn' | 'ok' | 'missing' }) {
  const icon =
    tone === 'ok'      ? '✓' :
    tone === 'warn'    ? '⚠' :
    tone === 'missing' ? '⊘' :
  ...
  const color =
    ...
    tone === 'missing' ? '#fb923c'        :   // bright orange — distinct from red errors
    'var(--red)'
  return (
    <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start', padding: '5px 0', fontSize: '0.75rem' }}>
      <span style={{ color, flexShrink: 0 }}>{icon}</span>
      <span style={{ color: tone === 'missing' ? '#fb923c' : 'var(--text)', lineHeight: 1.5 }}>{text}</span>
    </div>
  )
}
...
// Path row (for bb-scan results)
export function PathRow({ path, status }: { path: string; status: number }) {
  const color = status === 200 ? 'var(--lime)' : status < 400 ? 'var(--yellow)' : 'var(--red)'
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '4px 8px', borderRadius: 4, marginBottom: 3, background: 'var(--surface2)', fontSize: '0.73rem' }}>
...
export function PayloadItem({ payload, label }: { payload: string; label: string }) {
  ...
  return (
    ...
    >
      <code style={{ color: 'var(--lime)', fontSize: '0.72rem', wordBreak: 'break-all' as const, flex: 1 }}>{payload}</code>
    ...
    <div style={{ border: '1px solid var(--border)', borderRadius: 'var(--radius)', overflow: 'hidden', marginBottom: 12 }}>
      <button
        ...
        style={{
          ...
          fontFamily: "'JetBrains Mono', monospace", fontSize: '0.72rem', letterSpacing: 1, textTransform: 'uppercase',
        }}
      >
        <span>{title}</span>
