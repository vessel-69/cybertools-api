  ...
  const recon =
    cmd === "recon"
      ...
      : cmd === "workflow"
        ...
        : cmd === "workflow-express"
          ? (data as ExpressWorkflowResult).recon
          : cmd === "workflow-bugbounty"
            ? (data as BugBountyWorkflowResult).recon
            : null;
...
export default function App() {
  ...
  async function run(cmd: Command) {
    ...
    try {
      ...
      switch (cmd) {
        case "recon":
          data = await api.recon(target);
          break;
        case "analyze":
          data = await api.analyze(url);
          break;
        case "bb-scan":
          data = await api.bbScan(url);
          break;
        case "workflow":
          data = await api.workflow(target);
          break;
        case "payloads":
          data = await api.payloads(payloadType);
          break;
        case "last-scan":
          data = await api.lastScan();
          break;
        case "expand":
          data = await api.expand(target);
          break;
        case "endpoints":
          data = await api.endpoints(url);
          break;
        case "params":
          data = await api.params(url);
          break;
        case "workflow-express":
          data = await api.workflowExpress(target);
          break;
        case "workflow-bugbounty":
          data = await api.workflowBugBounty(target);
          break;
        case "workflow-subdomains":
          data = await api.workflowSubdomains(target);
          break;
        case "workflow-api":
          data = await api.workflowApi(url);
          break;
        default:
          return;
      }
