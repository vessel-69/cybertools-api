...
def _get(path):
    try:
        req = urllib.request.Request(
            BASE_URL + path, headers={"User-Agent": "CyberTools-CLI/2.0"}
        )
    ...
    if found:
        for p in found:
            color = (
                G
                if p["status"] == 200
                else Y if str(p["status"]).startswith("3") else RE
            )
...
def cmd_workflow(target):
    ...
    for p in found[:8]:
        color = (
            G if p["status"] == 200 else Y if str(p["status"]).startswith("3") else RE
        )
        _row(p["path"], p["status"], color)
